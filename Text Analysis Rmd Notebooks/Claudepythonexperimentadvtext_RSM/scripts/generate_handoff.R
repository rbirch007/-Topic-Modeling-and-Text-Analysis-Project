# generate_handoff.R  (RELIEF SOCIETY MAGAZINE) -----------------------------
# Bridge between the RSM topic linker (Topic_to_Article_Linker_RSM.Rmd) and the
# Windows advanced-analysis pipeline. Reads topic_linker_session_K{K}.RData and
# writes the row-aligned handoff artifacts the Python compute layer + the .Rmd
# consume. Identical to the WE version except BASE points at the RSM project.
#
# Neither K nor scenario is hardcoded:
#   Rscript generate_handoff.R                   -> newest session, scenario "RSMfull"
#   Rscript generate_handoff.R 18                -> force K=18, scenario "RSMfull"
#   Rscript generate_handoff.R 18 RSM1951-1970   -> force K=18, scenario RSM1951-1970
#   set ADV_SCENARIO=RSM1935-1950; Rscript generate_handoff.R 15  -> scenario via env
# (scenario: arg 2 > env ADV_SCENARIO > "RSMfull"; matches the linker + config.py)
#
...
BASE <- "C:/Users/birch/Rachel/GMU/Dissertation/textanalysis/Claudepythonexperimentadvtext_RSM"

# --- resolve scenario + K ---------------------------------------------------
args <- commandArgs(trailingOnly = TRUE)

# scenario: arg 2 > env ADV_SCENARIO > "RSMfull". Each scenario is its own
# output_linked/<scenario>/ subfolder, matching the linker and config.py.
scenario <- if (length(args) >= 2 && nzchar(args[2])) {
  args[2]
} else {
  env_s <- Sys.getenv("ADV_SCENARIO"); if (nzchar(env_s)) env_s else "RSMfull"
}
