#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 8 (1921).

This volume features the serial fiction "Out of the World" by G. Milton Babcock,
the Watch Tower and World Happenings columns, and early Relief Society matters.
...
Usage:
    python extract_vol8.py
    python extract_vol8.py --dry-run
"""
...
from vol7_8_9_toc import VOL8_TOC
...
for no in range(1, 13):
    ...
    fname = f"Vol8_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol8", key)] = (fname, month)
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 8 (1921) into individual entries"
    )
    ...
    vol8_out = OUTPUT_DIR / "vol8"
    if vol8_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol8_out}")
        shutil.rmtree(vol8_out)
    ...
    for (vol, issue_key), entries in VOL8_TOC.items():
