#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 9 (1922).

This volume features the serial fiction "Bubbles and Troubles" by Ruth Moench Bell,
the General Procession column, and early Relief Society matters.
...
Usage:
    python extract_vol9.py
    python extract_vol9.py --dry-run
"""
...
from vol7_8_9_toc import VOL9_TOC
...
for no in range(1, 13):
    ...
    fname = f"Vol9_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol9", key)] = (fname, month)

# Override March filename: actual file on disk has typo "1822" instead of "1922"
ISSUE_FILES[("Vol9", "No3_March_1922")] = ("Vol9_No3_March_1822.txt", "March")
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 9 (1922) into individual entries"
    )
    ...
    vol9_out = OUTPUT_DIR / "vol9"
    if vol9_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol9_out}")
        shutil.rmtree(vol9_out)
    ...
    for (vol, issue_key), entries in VOL9_TOC.items():
