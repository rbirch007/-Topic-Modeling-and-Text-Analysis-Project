#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 4 (1917).

This early volume features wartime content, "Mothers in Israel" profiles,
"Home Science Department" sections, and "Guide Lessons."
...
Usage:
    python extract_vol4.py
    python extract_vol4.py --dry-run
"""
...
for no in range(1, 13):
    ...
    fname = f"Vol4_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol4", key)] = (fname, month)
...
# ---------------------------------------------------------------------------
# Known OCR-mangled section headers for Vol4.
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 4 into individual entries"
    )
    ...
    vol4_out = OUTPUT_DIR / "vol4"
    if vol4_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol4_out}")
        shutil.rmtree(vol4_out)
    ...
    for (vol, issue_key), entries in VOL4_TOC.items():
