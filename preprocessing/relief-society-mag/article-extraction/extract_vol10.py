#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 10 (1923).
...
Usage:
    python extract_vol10.py
    python extract_vol10.py --dry-run
"""
...
from vol10_11_12_toc import VOL10_TOC
...
for no in range(1, 13):
    ...
    fname = f"Vol10_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol10", key)] = (fname, month)
...
# ---------------------------------------------------------------------------
# Known OCR-mangled section headers for Vol10.
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 10 into individual entries"
    )
    ...
    vol10_out = OUTPUT_DIR / "vol10"
    if vol10_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol10_out}")
        shutil.rmtree(vol10_out)
    ...
    for (vol, issue_key), entries in VOL10_TOC.items():
