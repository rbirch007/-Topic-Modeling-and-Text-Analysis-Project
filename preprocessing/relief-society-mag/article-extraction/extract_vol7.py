#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 7 (1920).

This volume features the serial fiction "Beyond Arsareth" by Nephi Anderson,
content on suffrage, the Watch Tower column, and early Relief Society matters.
...
Usage:
    python extract_vol7.py
    python extract_vol7.py --dry-run
"""
...
from vol7_8_9_toc import VOL7_TOC
...
for no in range(1, 13):
    ...
    fname = f"Vol7_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol7", key)] = (fname, month)
...
# ---------------------------------------------------------------------------
# Known OCR-mangled section headers for Vol7.
# ---------------------------------------------------------------------------
# Serial fiction chapter pattern: Beyond Arsareth by Nephi Anderson
_BEYOND_ARSARETH_PAT = (
    r"(?:B|G)eyond\s+(?:A|G)rsareth"
    r".{0,80}?"
)
...
_KNOWN_HEADER_PATTERNS = {
    ...
    # Serial: Beyond Arsareth
    "Beyond Arsareth": _BEYOND_ARSARETH_PAT,
}
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 7 (1920) into individual entries"
    )
    ...
    vol7_out = OUTPUT_DIR / "vol7"
    if vol7_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol7_out}")
        shutil.rmtree(vol7_out)
    ...
    for (vol, issue_key), entries in VOL7_TOC.items():
