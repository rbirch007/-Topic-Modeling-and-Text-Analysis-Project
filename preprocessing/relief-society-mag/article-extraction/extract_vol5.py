#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 5 (1918).

This wartime volume features patriotic content, Red Cross coverage,
"Unusual Mothers" profiles, and "Guide Lessons."
...
Usage:
    python extract_vol5.py
    python extract_vol5.py --dry-run
"""
...
for no in range(1, 13):
    ...
    fname = f"Vol5_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol5", key)] = (fname, month)
...
# ---------------------------------------------------------------------------
# Known OCR-mangled section headers for Vol5.
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 5 into individual entries"
    )
    ...
    vol5_out = OUTPUT_DIR / "vol5"
    if vol5_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol5_out}")
        shutil.rmtree(vol5_out)
    ...
    for (vol, issue_key), entries in VOL5_TOC.items():
