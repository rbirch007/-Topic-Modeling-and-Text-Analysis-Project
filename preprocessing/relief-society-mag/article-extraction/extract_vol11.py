#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 11 (1924).
...
Usage:
    python extract_vol11.py
    python extract_vol11.py --dry-run
"""
...
from vol10_11_12_toc import VOL11_TOC
...
for no in range(1, 13):
    ...
    fname = f"Vol11_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol11", key)] = (fname, month)
...
# ---------------------------------------------------------------------------
# Known OCR-mangled section headers for Vol11.
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 11 into individual entries"
    )
    ...
    vol11_out = OUTPUT_DIR / "vol11"
    if vol11_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol11_out}")
        shutil.rmtree(vol11_out)
    ...
    for (vol, issue_key), entries in VOL11_TOC.items():
