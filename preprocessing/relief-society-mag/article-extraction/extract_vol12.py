#!/usr/bin/env python3
"""
Extraction script for Relief Society Magazine Volume 12 (1925).
...
Usage:
    python extract_vol12.py
    python extract_vol12.py --dry-run
"""
...
from vol10_11_12_toc import VOL12_TOC
...
for no in range(1, 13):
    ...
    fname = f"Vol12_No{no}_{month}_{year}.txt"
    ISSUE_FILES[("Vol12", key)] = (fname, month)
...
# ---------------------------------------------------------------------------
# Known OCR-mangled section headers for Vol12.
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Relief Society Magazine Vol 12 into individual entries"
    )
    ...
    vol12_out = OUTPUT_DIR / "vol12"
    if vol12_out.exists() and not args.dry_run:
        print(f"Cleaning output directory: {vol12_out}")
        shutil.rmtree(vol12_out)
    ...
    for (vol, issue_key), entries in VOL12_TOC.items():
