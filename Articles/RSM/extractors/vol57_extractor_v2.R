...
VOLUME  <- 57L
YEAR    <- 1970L
INPUT_DIR  <- "C:/Users/birch/OneDrive - George Mason University - O365 Production/Dissertation/textanalysis/Articleextractionrfiles/input/Vol57split"
    ...
    if (nrow(page_index) >= 3) {
      ...
      if (!is.na(pos)) {
        ...
        if (!is.na(match_page)) {
          ...
          # Vol57 v2: Relaxed thresholds (was 2/8/4, now 4/10/8)
          max_page_diff <- if (is_poem) 4.0
                           else if (is_fiction) 10.0
                           else 8.0
    ...
    if (length(saved_ranges) > 0) {
      ...
      for (gi in seq_along(gap_texts)) {
        ...
        if (!is.null(matched_file) && file.exists(matched_file)) {
          ...
          # Ad-trim gap text: only trim trailing ads.
          # Mid-article ad stripping (strip_mid_article_ads) is too aggressive
          # for gap text — it catches article content interleaved with ads on
          # OCR pages where ads and articles share the same page.  Better to
          # keep some ad text in articles than lose article text to Misc.
          # Step: Trim trailing ads that extend to the end of the gap
          gap_ad <- detect_ad_start(gt_clean, min_article_len = 50L)
          if (!is.na(gap_ad) && gap_ad > 50L) {
            gap_ad_content <- str_sub(gt_clean, gap_ad, nchar(gt_clean))
            gt_clean <- str_trim(str_sub(gt_clean, 1L, gap_ad - 1L))
            append_ads_to_misc(out_folder, prefix, gap_ad_content, "gap text")
          }
      ...
      if (length(misc_texts) > 0) {
        ...
        for (mti in seq_along(misc_texts)) {
          ...
          for (ti in seq_len(nrow(gap_title_hits))) {
            ...
            # Ad-trim: only trailing ads (mid-article strip disabled
            # to avoid losing article text interleaved with ads on OCR pages)
            t3_ad <- detect_ad_start(t3_text_clean, min_article_len = 100L)
            if (!is.na(t3_ad) && t3_ad > 100L) {
              t3_ad_content <- str_sub(t3_text_clean, t3_ad, nchar(t3_text_clean))
              t3_text_clean <- str_trim(str_sub(t3_text_clean, 1L, t3_ad - 1L))
              append_ads_to_misc(out_folder, prefix, t3_ad_content, "T3 recovery")
            }
...
if (length(month_dirs) > 0) {
  ...
  for (mdir in month_dirs) {
    ...
    # ---- REPAIR B: Overlap detection (diagnostic only) ----
    # Adjacent articles are checked for n-gram overlap.  Overlap is
    # LOGGED but NOT auto-stripped.  Stripping to Misc creates empty
    # stubs and loses article content; the QC DUPLICATE_CONTENT
    # warning flags these for downstream awareness instead.
