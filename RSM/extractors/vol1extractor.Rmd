...
process_vol2_file <- function(filepath, output_dir) {
...
```{r process}
input_files <- list.files(INPUT_DIR, pattern = "Vol2.*\\.txt$", full.names = TRUE)

cat("Found", length(input_files), "Volume 2 file(s)\n\n")

results <- lapply(input_files, process_vol_file, OUTPUT_DIR)
