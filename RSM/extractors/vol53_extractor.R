...
find_article_end <- function(body_text, start_pos, current_idx, arts,
  ...
  if (tier == "T1") {
    buffer <- if (is_poem) 4000L
              else if (category == "Frontispiece") 3000L
              else if (category %in% c("Fiction")) 10000L
  ...
  for (sec in sections) {
    ...
    if (sec$cat == "Lesson") {
      lesson_prefixes <- c("Theology", "Visiting Teacher",
                           "Work Meeting", "Homemaking",
                           "Literature", "Cultural Refinement",
...
process_vol50_issue <- function(filepath, output_dir) {
  ...
  # Find VOLUME masthead line (e.g., "VOLUME 55 FEBRUARY 1968 NUMBER 2")
  # Used to identify the boundary between pre-CONTENTS article content and metadata
  vol_masthead_idx <- NA_integer_
  for (i in seq_along(lines_raw)) {
    if (str_detect(lines_raw[i], regex("^VOLUME\\s+\\d+\\s+", ignore_case=TRUE))) {
      vol_masthead_idx <- i
      break
    }
  }
  ...
  if (!is_split_input) {
    ...
    # Prepend pre-CONTENTS/masthead article content to body.
    # The TOC is further down in the file; article content such as Frontispiece
    # poems and "From Near and Far" reader letters precedes it.
    pre_body_end <- if (!is.na(vol_masthead_idx)) vol_masthead_idx - 1L
                    else if (!is.na(contents_header_idx)) contents_header_idx - 1L
                    else 0L
    if (pre_body_end >= 2) {
      pre_lines <- lines_raw[1:pre_body_end]
      pre_lines <- pre_lines[nchar(str_trim(pre_lines)) > 0]
      pre_lines <- pre_lines[!str_detect(pre_lines, pub_skip_pattern)]
      pre_lines <- pre_lines[!str_detect(str_trim(pre_lines), "^\\d{1,3}\\s*$")]
      if (length(pre_lines) > 0) {
        pre_raw <- paste(pre_lines, collapse=" ")
        pre_raw <- str_replace_all(pre_raw, "\\s{2,}", " ")
        body_raw <- paste(pre_raw, body_raw, sep=" ")
        cat(sprintf("    Prepended %d pre-CONTENTS lines (%s chars) to body\n",
                    length(pre_lines), format(nchar(pre_raw), big.mark=",")))
      }
    }
  } else {
    ...
    # Concatenate body lines starting from line 1 (not body_line_idx).
    # The TOC is further down in the file; article content such as Frontispiece
    # poems and "From Near and Far" reader letters precedes it.  We include ALL
    # lines but EXCLUDE the CONTENTS/masthead zone to avoid false title matches.
    skip_start <- if (!is.na(vol_masthead_idx)) vol_masthead_idx
                  else if (!is.na(contents_header_idx)) contents_header_idx
                  else body_line_idx
    skip_end <- body_line_idx - 1L
    keep_indices <- if (skip_end >= skip_start)
                      setdiff(seq_len(n_lines), skip_start:skip_end)
                    else seq_len(n_lines)
    body_lines <- lines_raw[keep_indices]
    body_lines <- body_lines[nchar(str_trim(body_lines)) > 0]
    body_lines <- body_lines[!str_detect(str_trim(body_lines), "^\\d{1,3}\\s*$")]
    body_raw <- paste(body_lines, collapse=" ")
    # Clean up multiple spaces from joining
    body_raw <- str_replace_all(body_raw, "\\s{2,}", " ")
    cat(sprintf("    Including pre-CONTENTS content: lines 1-%d (excl. %d-%d)\n",
                n_lines, skip_start, skip_end))
  }
  ...
  # Always regenerate to ensure formatting fixes take effect
  contents_filepath <- file.path(out_folder, paste0(prefix, "_00_CONTENTS.txt"))
  if (!is.null(contents_text) && nchar(contents_text) > 10) {
    ...
    # The page number stays at the END of the current entry's line, and
    # the next title starts on a new line.
    main_section <- str_replace_all(main_section,
      "(\\d{1,3})(\\s+[\"'\\u201c]?[A-Z])", "\\1\n\\2")
    main_section <- str_replace_all(main_section,
      "(\\d{1,3}\\.)(\\s+[\"'\\u201c]?[A-Z])", "\\1\n\\2")
    ...
    for (fi in fp_idx) {
      ...
      if (split_found && !is.null(title2)) {
        ...
        new_rows[[length(new_rows)+1]] <- data.frame(
          ...
          category = if (str_detect(cl, regex("Frontispiece", ignore_case=TRUE))) "Frontispiece" else "Poetry", stringsAsFactors = FALSE)
        new_rows[[length(new_rows)+1]] <- data.frame(
  ...
  # ----------------------------------------------------------
  # Step 8a-fix: Adjust title positions using page_index.
  # Two scenarios:
  # A) BACKUP: Title found AFTER page start (OCR placed headline
  #    after opening content) → back up to page start.
  # B) ADVANCE: Title found >0.6 page BEFORE expected page (title
  #    appeared in a preamble/announcement before the actual article)
  #    → search again near expected page for a closer match; if none,
  #    advance to page_est.
  # Uses interpolated page_to_pos() so articles whose exact page
  # isn't in the running-header index still benefit.
  # ----------------------------------------------------------
  if (nrow(page_index) > 0) {
    adjust_count <- 0L
    for (i in seq_along(positions)) {
      if (!is.na(positions[i]) && search_method[i] == "title") {
        art_page <- arts$page[i]
        if (is.na(art_page)) next

        # Get page position: exact match first, then interpolation
        if (art_page %in% page_index$page) {
          page_pos <- page_index$position[page_index$page == art_page]
        } else {
          page_pos <- page_to_pos(page_index, art_page, body_start, body_end)
        }
        if (is.na(page_pos)) next

        gap <- positions[i] - page_pos

        # A) BACKUP: title found 50-5000 chars AFTER page start
        if (gap > 50 && gap < 5000) {
          cat(sprintf("    BACKUP: '%s' title@%d page%d@%d (-%d chars)\n",
                      str_sub(arts$title[i], 1, 40),
                      positions[i], art_page, page_pos, gap))
          positions[i] <- page_pos
          adjust_count <- adjust_count + 1L
        }

        # B) ADVANCE: title found >0.6 page BEFORE expected page
        #    (e.g., title mentioned in contest announcement or preamble)
        if (gap < -1500) {
          old_pos <- positions[i]
          # Search for title again near the expected page
          adv_start <- max(body_start, page_pos - 500L)
          adv_end   <- min(body_end,   page_pos + 5000L)
          new_pos <- find_in_window(body_text, arts$title[i],
                                   adv_start, adv_end, allow_s10 = TRUE)
          if (!is.na(new_pos) && new_pos != old_pos) {
            cat(sprintf("    ADVANCE: '%s' old@%d new@%d page%d (moved +%d)\n",
                        str_sub(arts$title[i], 1, 40), old_pos, new_pos,
                        art_page, new_pos - old_pos))
            positions[i] <- new_pos
            adjust_count <- adjust_count + 1L
          } else {
            # No closer match; use page position as estimate
            cat(sprintf("    ADVANCE_EST: '%s' title@%d -> page%d@%d\n",
                        str_sub(arts$title[i], 1, 40), old_pos,
                        art_page, page_pos))
            positions[i] <- page_pos
            search_method[i] <- "page_est"
            adjust_count <- adjust_count + 1L
    ...
    if (adjust_count > 0)
      cat(sprintf("  Adjusted %d title positions using page_index\n", adjust_count))
  }
  ...
  if (length(misc_fp_idx) > 0 && exists("misc_text") && nchar(misc_text) >= 30) {
    ...
    for (i in misc_fp_idx) {
      ...
      fp_text <- ""
      if (length(ftw) >= 1) {
        # Find the ALL-CAPS title in misc_text (poem headers are typically all-caps)
        caps_title <- toupper(paste(ftw[1:min(3, length(ftw))], collapse = " "))
        fp_loc <- str_locate(misc_text, fixed(caps_title))
        if (is.na(fp_loc[1,1])) {
          # Fallback: find first significant title word
          fp_loc <- str_locate(misc_text, fixed(ftw[1], ignore_case = TRUE))
        }
        if (!is.na(fp_loc[1,1])) {
          # Start extraction from the title match position
          fp_start <- fp_loc[1,1]
          # End at the author name line or section break
          fp_chunk <- str_sub(misc_text, fp_start, min(nchar(misc_text), fp_start + 2000L))
          # Look for section breaks: metadata lines (page numbers, "Lithographed",
          # "Transparency"), double newlines before a new section
          break_pats <- c(
            "\\d{3}\\s*$",           # page number at end of poem section
            "(?i)Transparency",      # photo credit line
  ...
  for (mi in miss_idx) {
    ...
    if (str_detect(misc_text, regex("[\u2014\u2013—-]\\s*[A-Z][a-z]+.*(?:Utah|California|Idaho|England|Canada|Washington|Oregon|Arizona)",
      ...
      if (nchar(remaining_misc) > 10) {
        ...
        # Keep the Misc file with publishing boilerplate so ads have a destination
        cat("    FNF extracted from Misc (Misc file preserved for ads)\n")
      }
    }
  }
    ...
    if (length(saved_ranges) > 0) {
      ...
      for (gi in seq_along(gap_texts)) {
        ...
        if (!is.null(matched_file) && file.exists(matched_file)) {
          ...
          # Better to keep some ad text in articles than lose article text to Misc.
