# rsm_extractor_core.R - Shared extraction functions for Relief Society Magazine
# Volumes 50-57 (1963-1970)
# Source this file from per-volume Rmd/R scripts after setting config vars.
#
# Required config vars (set before sourcing):
#   VOLUME  - integer volume number
#   YEAR    - integer year
#   INPUT_DIR  - path to input split files
#   OUTPUT_DIR - path to output directory
#   SPLIT_INPUT_LINES - TRUE/FALSE for mega-line splitting
#
# To customize a function for a specific volume, redefine it
# in your Rmd AFTER the source() call.
...
find_article_end <- function(body_text, start_pos, current_idx, arts,
  ...
  # Tightened buffers to reduce boundary bleed between adjacent articles.
  # T1 articles have confirmed title positions; T2 use tighter buffers.
  if (tier == "T1") {
    buffer <- if (is_poem) 2500L
              else if (category %in% c("Fiction")) 8000L
              else if (category %in% c("Lesson", "Home")) 5000L
              else 3500L
  } else {
    buffer <- if (is_poem) 1200L
              else if (category %in% c("Fiction")) 3500L
              else if (category %in% c("Lesson", "Home")) 2000L
              else 1500L
  }
  ...
  # --- Signal 3b: Lesson/Section header boundary ---
  # "Lesson Department" followed by a department name is a strong section
  # boundary in the Relief Society Magazine. This catches cases where the
  # next CONTENTS article is T2 (page_est) and not in the title_pos_map,
  # preventing articles like Notes/Activities from absorbing entire lesson sections.
  # Only fires when the current article is NOT itself a lesson/department article.
  is_lesson_art <- str_detect(arts$title[current_idx],
    regex("Lesson|Homemaking|Spiritual|Visiting|Social|Cultural|Refinement",
          ignore_case = TRUE))
  if (!is_lesson_art && !is_poem) {
    section_hdr_pat <- "Lesson\\s+Department\\s+(?:HOMEMAKING|SPIRITUAL|VISITING|SOCIAL|CULTURAL)"
    section_chunk <- str_sub(body_text, start_pos + min_gap, search_end)
    section_locs <- str_locate_all(section_chunk, regex(section_hdr_pat))[[1]]
    if (nrow(section_locs) > 0) {
      section_pos <- start_pos + min_gap + section_locs[1, 1] - 1L
      # Only use if it's well past the article start (not the article itself)
      if (section_pos > start_pos + 2000L) {
        next_title_pos <- min(c(next_title_pos, section_pos), na.rm = TRUE)
        cat(sprintf("    [SECTION_BOUNDARY] #%d '%s': 'Lesson Department' header at pos %d\n",
                    current_idx, str_sub(arts$title[current_idx], 1, 40), section_pos))
      }
    }
  }
  ...
  # --- Signal 5: Embedded page number boundary ---
  # OCR text often contains inline page numbers (e.g., "26" or "755") at page
  # transitions. When the next CONTENTS article's page number appears as a
  # standalone number in the text, it marks where that article's page begins.
  # This is more precise than the page-to-position estimation.
  inline_pg_pos <- NA_integer_
  if (!is.na(next_page) && next_page > article_page && !is_poem) {
    # Search for the next page number appearing as a standalone number
    # (preceded/followed by whitespace or line boundaries)
    pg_pat <- sprintf("(?<=\\s|^)%d(?=\\s|$)", next_page)
    search_chunk <- str_sub(body_text, start_pos + min_gap,
                            min(page_cap, start_pos + 30000L))
    pg_locs <- str_locate_all(search_chunk, regex(pg_pat))[[1]]
    if (nrow(pg_locs) > 0) {
      # Use the first occurrence that's past the minimum article length
      for (pli in seq_len(nrow(pg_locs))) {
        cand <- start_pos + min_gap + pg_locs[pli, 1] - 1L
        if (cand > start_pos + 500L) {  # at least 500 chars of article content
          inline_pg_pos <- cand
          break
        }
      }
  ...
  all_candidates <- c(cont_end, next_title_pos, ad_pos, page_cap, inline_pg_pos)
...
parse_contents_multiline <- function(lines) {
  ...
  # AND multi-entry segments. Some OCR CONTENTS pages jam multiple
  # category sections onto one line, e.g., "...Author Fiction 334 Title..."
  # Step 1: split at inline category boundaries
  # Step 2: split multi-entry segments at page-number boundaries
  split_at_pages <- function(seg) {
    # Split a segment at page-number boundaries: "334 Title Author 346 Title..."
    # becomes c("334 Title Author", "346 Title...")
    # Pattern: space + 2-3 digit number + space + capital letter (new entry)
    pg_locs <- str_locate_all(seg, "(?<=\\s)(\\d{2,3})\\s+[A-Z]")[[1]]
    if (nrow(pg_locs) <= 1) return(seg)  # 0 or 1 page number = single entry
    result <- character(0)
    ...
    # Include any text before the first page number (could be category header + entry)
    if (pg_locs[1, 1] > 1) {
      pre <- str_trim(str_sub(seg, 1, pg_locs[1, 1] - 1L))
      if (nchar(pre) >= 3) result <- c(pre, result)
    }
    result
  }

  expanded_lines <- character(0)
  for (line in lines) {
    lt <- str_trim(line)
    if (nchar(lt) > 100) {
      inline_cats <- str_locate_all(lt, regex(cat_pattern, ignore_case = TRUE))[[1]]
      if (nrow(inline_cats) > 0) {
        # Keep only matches NOT at position 1 (start-of-line cats handled later)
        mid <- inline_cats[inline_cats[, 1] > 1, , drop = FALSE]
        if (nrow(mid) > 0) {
          prev <- 1L
          for (ci in seq_len(nrow(mid))) {
            before <- str_trim(str_sub(lt, prev, mid[ci, 1] - 1L))
            if (nchar(before) >= 3) {
              expanded_lines <- c(expanded_lines, split_at_pages(before))
            }
            prev <- mid[ci, 1]
          }
          tail_text <- str_trim(str_sub(lt, prev, nchar(lt)))
          if (nchar(tail_text) >= 3) {
            expanded_lines <- c(expanded_lines, split_at_pages(tail_text))
          }
          cat(sprintf("    INLINE_CAT_SPLIT: split %d-char line into %d parts at %d inline headers\n",
                      nchar(lt), length(expanded_lines), nrow(mid)))
          next
        }
      }
      # Long line without category headers but possibly multiple entries
      expanded_lines <- c(expanded_lines, split_at_pages(lt))
      next
    }
    expanded_lines <- c(expanded_lines, lt)
  }
...
read_contents_file <- function(filepath) {
  ...
  rc_cat_pat <- "(?:SPECIAL FEATURES?|FICTION|GENERAL FEATURES?|THE HOME\\s*[-—:].{0,30}|LESSON\\s*DEPARTMENT|POETRY|FEATURE.{0,5}HOME|LESSONS?\\s+FOR\\s+\\w+)"
  ...
  for (line in file_lines) {
    ...
    # Non-poetry entries: "Title Author page_num" (page at END, Vol51-52 format)
    em_end <- str_match(line, "^(.+?)\\s+(\\d{1,3})\\s*$")
    if (!is.na(em_end[1,1])) {
      title_text <- str_trim(em_end[1,2])
      pg <- as.integer(em_end[1,3])
      # Skip if title text looks like a category header alone
      if (!str_detect(title_text, regex("^(Special Features?|Fiction|General Features?|The Home|Lessons?|Poetry|Appointments)$",
                                         ignore_case=TRUE))) {
        arts <- rbind(arts, data.frame(
          title=title_text, page=pg, category=current_cat,
          stringsAsFactors=FALSE))
...
clean_ocr_body <- function(txt) {
  ...
  txt <- str_replace_all(txt, "\n\\s*VOLUME\\s*\n", "\n\n")
  # Strip Volume masthead patterns: "Volume N MONTH YEAR Number N"
  # These appear when title pages are OCR-merged into article text.
  for (mu in MONTH_UPPER) {
    txt <- str_replace_all(txt,
      regex(paste0("Volume\\s+\\d+\\s+", mu, "\\s+", YEAR, "\\s+Number\\s+\\d+"),
            ignore_case = TRUE),
      "\n\n")
  }
  # Strip embedded Board officer listings (between masthead and article text)
  txt <- str_replace_all(txt,
    regex("THE GENERAL BOARD.{0,800}?(?:Secretary[- ]?Treasurer|Miltenberger)",
...
process_vol50_issue <- function(filepath, output_dir) {
  ...
  if (!is.na(contents_header_idx) && body_line_idx > contents_header_idx + 1) {
  ...
  min_pg <- (month_num - 1L) * 80L + 1L
  max_pg <- month_num * 80L + 20L  # allow some overflow
  ...
  rh_locs <- str_locate_all(body_raw, regex(rh_pat, ignore_case=TRUE))[[1]]
  if (nrow(rh_locs) > 0) {
    # Build replacement string, skipping headers preceded by "Volume" / "Vol"
    # to avoid splitting article text that references a specific volume.
    parts <- character(0)
    prev_end <- 1L
    rh_inserted <- 0L
    rh_skipped  <- 0L
    for (ri in seq_len(nrow(rh_locs))) {
      s <- rh_locs[ri, 1]; e <- rh_locs[ri, 2]
      # Check 30 chars before this header for "Volume" / "Vol" context
      lookback <- str_sub(body_raw, max(1L, s - 30L), s - 1L)
      if (str_detect(lookback, regex("(?:Volume|Vol\\.?)\\s*$", ignore_case=TRUE))) {
        # This is a volume reference, not a running header — skip
        rh_skipped <- rh_skipped + 1L
        next
      }
      header_text <- str_sub(body_raw, s, e)
      parts <- c(parts, str_sub(body_raw, prev_end, s - 1L),
                 paste0("\n\n", header_text, "\n\n"))
      prev_end <- e + 1L
      rh_inserted <- rh_inserted + 1L
    }
    parts <- c(parts, str_sub(body_raw, prev_end, nchar(body_raw)))
    body_text <- paste0(parts, collapse="")
    cat(sprintf("  Inserted %d running header breaks", rh_inserted))
    if (rh_skipped > 0) cat(sprintf(" (skipped %d Volume refs)", rh_skipped))
    cat("\n")
  } else {
    body_text <- body_raw
  ...
  # ----------------------------------------------------------
  # Step 4a: Strip embedded Volume masthead + Board listing
  # Some OCR scans embed a title page (Volume N MONTH YEAR Number N
  # + THE GENERAL BOARD officers) in the MIDDLE of body text.
  # This disrupts article boundaries and must be removed.
  # The pattern: "Volume N MONTH YEAR Number N" followed by
  # editor/board officer listings up to "SecretaryTreasurer" or "Secretary-Treasurer".
  # ----------------------------------------------------------
  masthead_pat <- paste0(
    "Volume\\s+\\d+\\s+", month_upper, "\\s+", YEAR,
    "\\s+Number\\s+\\d+",
    "(?:\\s+(?:THE\\s+GENERAL\\s+BOARD|Editor|Associate\\s+Editor|General\\s+Manager)",
    ".{0,800}?",
    "(?:Secretary[- ]?Treasurer|Miltenberger))?")
  mh_locs <- str_locate_all(body_text, regex(masthead_pat, ignore_case = TRUE, dotall = TRUE))[[1]]
  if (nrow(mh_locs) > 0) {
    # Only strip mastheads that appear in the body (not in pre-body/header area)
    mh_stripped <- 0L
    for (mi in seq_len(nrow(mh_locs))) {
      mh_s <- mh_locs[mi, 1]; mh_e <- mh_locs[mi, 2]
      # Skip if this is at the very start of body text (likely legitimate header)
      if (mh_s < 500L) next
      mh_text <- str_sub(body_text, mh_s, mh_e)
      body_text <- paste0(
        str_sub(body_text, 1L, mh_s - 1L),
        "\n\n",
        str_sub(body_text, mh_e + 1L, nchar(body_text))
      )
      mh_stripped <- mh_stripped + 1L
      cat(sprintf("  MASTHEAD STRIP: removed %d chars at pos %d (Volume masthead + Board listing)\n",
                  nchar(mh_text), mh_s))
    }
    if (mh_stripped > 0) {
  ...
  for (i in seq_len(nrow(arts))) {
    ...
    pos <- NA_integer_
    is_fiction_art <- isTRUE(arts$category[i] == "Fiction")
    is_editorial_art <- str_detect(title, regex("^Editorial", ignore_case = TRUE))
    if (nrow(page_index) >= 3) {
      # Primary search: article's page range with standard buffer
      # Fiction gets larger buffer (3500) because chapter headings often appear
      # on the page BEFORE the listed page (e.g., heading on pg 659 for a
      # chapter listed at pg 660). The coarse page index can mis-estimate
      # positions by 1-2 pages, causing the heading to fall outside a 1500-char
      # buffer window.  A wider buffer ensures the real heading is in range.
      # Editorials get a wider buffer (3000) because the OCR page number near
      # the editorial heading can differ by 1 from the CONTENTS page number,
      # pushing the heading just outside the standard window.
      primary_buf <- if (is_fiction_art) 3500L
                     else if (is_editorial_art) 3000L
                     else 1500L
      ...
      if (is.na(pos)) {
        wider_buf <- if (is_fiction_art) 7000L
                     else if (is_editorial_art) 6000L
                     else 4000L
        pr2 <- get_article_page_range(arts, i, page_index, body_start, body_end,
                                      buffer = wider_buf)
        pos <- find_in_window(body_text, title, pr2$start, pr2$end, allow_s10=TRUE)
      }

      # Page-position validation: reject matches whose estimated page
      # is too far from the CONTENTS page number.
      # Poems: ±2 pages. Fiction: ±8 pages (multi-page chapters).
      # Other articles: ±4 pages.
      if (!is.na(pos)) {
        match_page <- pos_to_page(page_index, pos)
        if (!is.na(match_page)) {
          page_diff <- abs(match_page - pg)
          is_poem <- isTRUE(arts$category[i] == "Poetry")
          is_fiction <- isTRUE(arts$category[i] == "Fiction")
          max_page_diff <- if (is_poem) 2.0
                           else if (is_fiction) 8.0
                           else 4.0
          if (page_diff > max_page_diff) {
            cat(sprintf("    [PAGE_REJECT] #%d '%s' (pg %d): match at page ~%.1f (diff %.1f > %.1f)\n",
                        i, str_sub(title, 1, 40), pg, match_page, page_diff, max_page_diff))
            pos <- NA_integer_
          }
        }
      }
      ...
      if (!is.na(pos)) {
        ...
        # Before PAGE_EST: check if a poem's title appears in pre-body/misc text.
        # Frontispiece poems are often in cover text (pre-body), not in body text,
        # but aren't always labeled "Frontispiece" in CONTENTS.
        # Strip author from title first, then check title words are close
        # together in misc_text (within 300 chars).
        if (is.na(pos) && isTRUE(arts$category[i] == "Poetry") &&
            exists("misc_text") && nchar(misc_text) >= 30) {
          # Strip author from title: "Title Words, Author Name" -> "Title Words"
          title_only <- str_replace(title,
            "\\s*,?\\s+[A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){0,3}$", "")
          if (nchar(title_only) < 4) title_only <- title  # safety
          ptw <- get_significant_words(title_only)
          ptw <- ptw[nchar(ptw) >= 4]
          if (length(ptw) >= 2) {
            n_cm <- min(3L, length(ptw))
            # Find position of each word in misc_text
            word_locs <- sapply(ptw[1:n_cm], function(w) {
              locs <- str_locate(misc_text, fixed(w, ignore_case = TRUE))
              if (!is.na(locs[1, 1])) locs[1, 1] else NA_integer_
            })
            word_locs <- word_locs[!is.na(word_locs)]
            misc_ph <- length(word_locs)
            # Proximity check: matched words must appear within 300 chars
            words_proximate <- (misc_ph >= 2 &&
              (max(word_locs) - min(word_locs)) <= 300L)
            if (words_proximate) {
              positions[i] <- body_start
              search_method[i] <- "misc_frontispiece"
              pos <- body_start  # prevent PAGE_EST fallback
              cat(sprintf("    [POEM_IN_MISC] #%d '%s' (pg %d): %d/%d title words in pre-body text (span %d chars)\n",
                          i, str_sub(title, 1, 40), pg, misc_ph, n_cm,
                          max(word_locs) - min(word_locs)))
            }
          }
        }
        ...
        if (is.na(pos)) {
          ...
          if (!is.na(est)) {
            # For poems: search for verse-like structure near estimated position
            if (isTRUE(arts$category[i] == "Poetry")) {
              # Scan ±2 pages (4800 chars) around estimate for short-line clusters
              scan_start <- max(est - 4800L, body_start)
              scan_end   <- min(est + 4800L, body_end)
              scan_text  <- str_sub(body_text, scan_start, scan_end)
              # Split into 400-char windows, score each for verse structure
              best_verse_pos <- NA_integer_
              best_verse_score <- 0
              for (woff in seq(1, max(1, nchar(scan_text) - 400), by = 200)) {
                w <- str_sub(scan_text, woff, min(woff + 399L, nchar(scan_text)))
                lines_w <- str_split(w, "\n")[[1]]
                lines_w <- lines_w[nchar(str_trim(lines_w)) > 0]
                if (length(lines_w) < 3) next
                short_lines <- sum(nchar(str_trim(lines_w)) < 60)
                verse_score <- short_lines / length(lines_w)
                if (verse_score > 0.6 && verse_score > best_verse_score) {
                  best_verse_score <- verse_score
                  best_verse_pos <- scan_start + woff - 1L
                }
              }
              if (!is.na(best_verse_pos)) {
                positions[i] <- max(snap_to_para_start(body_text, best_verse_pos, body_start), body_start)
                search_method[i] <- "page_est_verse"
                cat(sprintf("    [PAGE_EST_VERSE] #%d '%s' (pg %d) -> pos %d (verse score %.2f)\n",
                            i, str_sub(title, 1, 40), pg, positions[i], best_verse_score))
              } else {
  ...
  # Sequential position enforcement: T1 positions must be monotonically
  # increasing with respect to CONTENTS order. Out-of-order T1 positions
  # indicate false matches — demote them to PAGE_EST.
  t1_idx <- which(search_method == "title")
  if (length(t1_idx) >= 2) {
    t1_positions <- positions[t1_idx]
    for (k in seq_along(t1_idx)[-1]) {
      if (!is.na(t1_positions[k]) && !is.na(t1_positions[k-1])) {
        if (t1_positions[k] <= t1_positions[k-1]) {
          orig_i <- t1_idx[k]
          est <- page_to_pos(page_index, arts$page[orig_i], body_start, body_end)
          cat(sprintf("    [MONO_DEMOTE] #%d '%s' (pg %d): pos %d <= prev %d — demoting to PAGE_EST at %d\n",
                      orig_i, str_sub(arts$title[orig_i], 1, 40), arts$page[orig_i],
                      t1_positions[k], t1_positions[k-1], est))
          positions[orig_i] <- max(snap_to_para_start(body_text, est, body_start), body_start)
          search_method[orig_i] <- "page_est"
          # Update tracking array for subsequent checks
          t1_positions[k] <- positions[orig_i]
        }
      }
    }
  }
  ...
  # ----------------------------------------------------------
  # Page index enrichment with T1 positions
  # ----------------------------------------------------------
  # T1 title-found articles have known positions AND known page
  # numbers from CONTENTS.  Adding these as page-index markers
  # dramatically improves PAGE_EST interpolation when the
  # running-header page index is sparse (e.g. March: 10 markers).
  t1_enrich_idx <- which(search_method == "title" & !is.na(positions) & arts$page > 0)
  if (length(t1_enrich_idx) >= 2) {
    enrich_df <- data.frame(
      page     = arts$page[t1_enrich_idx],
      position = positions[t1_enrich_idx],
      stringsAsFactors = FALSE
    )
    combined_pi <- rbind(page_index, enrich_df)
    combined_pi <- combined_pi[!duplicated(combined_pi$page), ]
    combined_pi <- combined_pi[order(combined_pi$position), ]

    if (nrow(combined_pi) > nrow(page_index)) {
      n_added <- nrow(combined_pi) - nrow(page_index)
      cat(sprintf("  PAGE INDEX ENRICH: +%d T1 markers (%d -> %d total)\n",
                  n_added, nrow(page_index), nrow(combined_pi)))

      pest_recalc <- which(search_method %in% c("page_est", "page_est_verse"))
      for (ri in pest_recalc) {
        old_pos <- positions[ri]
        new_est <- page_to_pos(combined_pi, arts$page[ri], body_start, body_end)
        if (!is.na(new_est)) {
          new_pos <- max(snap_to_para_start(body_text, new_est, body_start), body_start)
          if (abs(new_pos - old_pos) > 500) {
            cat(sprintf("    [ENRICH] #%d '%s' (pg %d): %d -> %d (delta %+d)\n",
                        ri, str_sub(arts$title[ri], 1, 40), arts$page[ri],
                        old_pos, new_pos, new_pos - old_pos))
          }
          positions[ri] <- new_pos
        }
      }
      page_index <- combined_pi
    }
  }
  ...
  unmark_used <- function(s, e) {
    s <- max(1L, as.integer(s)); e <- min(body_end, as.integer(e))
    if (e >= s) body_used[s:e] <<- FALSE
  }
  ...
  save_article <- function(i, art_text, tier_label,
    ...
    if (!is.na(ad_start) && ad_start > 100L) {
      ad_content <- str_sub(art_text, ad_start, nchar(art_text))
      cat(sprintf("    AD TRIM: '%s' trimmed %d ad chars → Misc\n",
                  str_sub(arts$title[i], 1, 40), nchar(ad_content)))
      art_text <- str_trim(str_sub(art_text, 1L, ad_start - 1L))
      append_ads_to_misc(out_folder, prefix, ad_content,
                         str_sub(arts$title[i], 1, 50))
    }
    ...
    if (arts$category[i] != "Poetry" && nchar(art_text) < 3000L &&
      ...
      if (ad_score >= 7L) {
        cat(sprintf("    AD REJECT: '%s' — ad score %d in first %d chars (%d total chars)\n",
    ...
    # FIXED: Excess text is NOT sent to Misc as "ADVERTISEMENTS". Instead, the
    # excess range in body_used is unmarked so gap detection can assign it to
    # the correct neighboring article (usually the NEXT article in sequence).
    if (arts$category[i] == "Poetry" && nchar(art_text) > 2000L) {
      ...
      if (!is.na(poem_break[1,1]) && poem_break[1,1] > 30) {
        ...
        # Unmark the trimmed portion so gap detection can reassign it
        if (!is.na(body_start_pos) && !is.na(body_end_pos)) {
          kept_ratio <- nchar(art_text) / max(1L, old_len)
          trim_global <- body_start_pos + as.integer(kept_ratio * (body_end_pos - body_start_pos))
          unmark_used(trim_global, body_end_pos)
          cat(sprintf("    POEM EXCESS RELEASED: %d chars unmarked for gap reassignment\n",
                      body_end_pos - trim_global))
        }
      } else {
        old_len <- nchar(art_text)
        ...
        # Unmark the trimmed portion so gap detection can reassign it
        if (!is.na(body_start_pos) && !is.na(body_end_pos)) {
          kept_ratio <- 1500.0 / max(1L, old_len)
          trim_global <- body_start_pos + as.integer(kept_ratio * (body_end_pos - body_start_pos))
          unmark_used(trim_global, body_end_pos)
          cat(sprintf("    POEM EXCESS RELEASED: %d chars unmarked for gap reassignment\n",
                      body_end_pos - trim_global))
          ...
          if (hi_hits >= 2L) {
            ...
            for (hw in hi_words) {
              ...
              if (!is.na(hloc[1,1])) {
                ...
                removed_chars <- old_poem_len - nchar(art_text)
                cat(sprintf("    POEM NEXT-ART TRIM: '%s' removed %d chars (next art: '%s')\n",
                            str_sub(arts$title[i], 1, 30), removed_chars,
                            str_sub(arts$title[hi], 1, 30)))
                # Unmark removed portion so gap detection assigns it to next article
                if (!is.na(body_start_pos) && !is.na(body_end_pos)) {
                  kept_ratio <- nchar(art_text) / max(1L, old_poem_len)
                  trim_global <- body_start_pos + as.integer(kept_ratio * (body_end_pos - body_start_pos))
                  unmark_used(trim_global, body_end_pos)
                }
                break
    ...
    # Build metadata bracket header: split title from author
    meta_title <- arts$title[i]
    meta_author <- ""
    auth_split <- str_match(arts$title[i],
      "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
    if (!is.na(auth_split[1,1])) {
      meta_title <- str_trim(auth_split[1,2])
      meta_author <- str_trim(auth_split[1,3])
    }
    meta_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                           VOLUME, meta$month, YEAR, arts$page[i],
                           arts$category[i], meta_title, meta_author)
    art_text <- paste0(meta_header, art_text)
  ...
  if (length(saved_ranges) > 0) {
    for (sr_i in seq_along(saved_ranges)) {
      ...
      if (!is.na(cont_ad) && cont_ad > 50L) {
        cont_ad_content <- str_sub(cont_text, cont_ad, nchar(cont_text))
        cont_text <- str_trim(str_sub(cont_text, 1L, cont_ad - 1L))
        append_ads_to_misc(out_folder, prefix, cont_ad_content,
                           paste0("continuation of ", basename(sr$file)))
      }
  ...
  misc_fp_extracted_text <- character(0)  # track extracted text for FNF dedup
  if (length(misc_fp_idx) > 0 && exists("misc_text") && nchar(misc_text) >= 30) {
    ...
    for (i in misc_fp_idx) {
      # Extract the frontispiece poem from misc_text (pre-body/cover text).
      # Strategy 1: Find poem via author name near "Frontispiece:"/"Art Layout:" markers
      # Strategy 2: Fallback to title word search (original method)
      ...
      # Strategy 1: Find author name near cover/frontispiece marker
      # The pre-body text pattern is: "Frontispiece: Art Layout: Author Name [POEM TEXT]
      #   [photo credits] [FNF reader letters]"
      # Poem starts RIGHT AFTER the author name in the cover credits
      author_last <- ""
      auth_m <- str_match(arts$title[i], "([A-Z][a-zA-Z]+)\\s*$")
      if (!is.na(auth_m[1,1])) author_last <- str_trim(auth_m[1,1])

      if (nchar(author_last) >= 3) {
        # Find "Frontispiece:" or "Art Layout:" marker
        marker_loc <- str_locate(misc_text,
          regex("(?:Frontispiece|Art Layout)\\s*:", ignore_case=TRUE))
        if (!is.na(marker_loc[1,1])) {
          # Find author name after the marker (within 200 chars)
          post_marker <- str_sub(misc_text, marker_loc[1,2],
                                 min(nchar(misc_text), marker_loc[1,2] + 200L))
          auth_in_post <- str_locate(post_marker, fixed(author_last, ignore_case=TRUE))
          if (!is.na(auth_in_post[1,1])) {
            # Poem starts after the author name
            poem_abs_start <- marker_loc[1,2] + auth_in_post[1,2]
            # Find poem end at photo credits or FNF start
            poem_chunk <- str_sub(misc_text, poem_abs_start,
                                  min(nchar(misc_text), poem_abs_start + 3000L))
            end_pats <- c(
              "(?i)Transparency\\s+by",         # photo credit
              "(?i)Lithographed",                # printing credit
              "(?i)Photograph\\s+by",            # photo credit
              "(?i)Dick Scopes",                 # recurring artist credit
              "(?i)Mary Scopes",                 # recurring artist credit
              "(?i)Springtime\\s+Blossoms",      # cover photo title
              "(?i)Relief Society Magazine",      # magazine name = FNF start
              "(?i)Volume\\s+\\d+\\s+Number",    # magazine metadata
              "(?i)\\d{4}\\s+Vol\\s+\\d+",       # year + volume
              "(?i)\\bVol\\.?\\s+\\d+,\\s+No",   # Vol 51, No. format
              "(?i)\\bI\\s+(?:enjoy|love|appreciate|have|am|want|congratulate|prize|cannot|wish|must)\\b"  # FNF letter start
            )
            best_end <- nchar(poem_chunk) + 1L
            for (ep in end_pats) {
              el <- str_locate(poem_chunk, regex(ep))
              if (!is.na(el[1,1]) && el[1,1] > 20 && el[1,1] < best_end) {
                best_end <- el[1,1]
              }
            }
            if (best_end <= nchar(poem_chunk)) {
              fp_text <- str_trim(str_sub(poem_chunk, 1, best_end - 1))
            } else {
              fp_text <- str_trim(str_sub(poem_chunk, 1, min(1500L, nchar(poem_chunk))))
            }
            if (nchar(fp_text) >= 20) {
              cat(sprintf("    FRNT Strategy 1: found poem after '%s' marker+author (%d chars)\n",
                          author_last, nchar(fp_text)))
            }
          }
        }
      }
      ...
      # Strategy 2: Fallback to title word search (original method)
      if (nchar(fp_text) < 20 && length(ftw) >= 1) {
        caps_title <- toupper(paste(ftw[1:min(3, length(ftw))], collapse = " "))
        fp_loc <- str_locate(misc_text, fixed(caps_title))
        if (is.na(fp_loc[1,1])) {
          fp_loc <- str_locate(misc_text, fixed(ftw[1], ignore_case = TRUE))
        }
        if (!is.na(fp_loc[1,1])) {
          fp_start <- fp_loc[1,1]
          ...
          break_pats <- c(
            ...
            "(?i)Volume\\s+\\d+\\s+Number",
            "(?i)\\d{4}\\s+Vol\\s+\\d+",
            "(?i)\\bVol\\.?\\s+\\d+,\\s+No",
            "(?i)\\bI\\s+(?:enjoy|love|appreciate|have|am|want|congratulate|prize|cannot|wish|must)\\b",
            "\n\\s*\n\\s*\n"
          )
          best_break <- nchar(fp_chunk) + 1L
          for (bp in break_pats) {
            bl <- str_locate(fp_chunk, regex(bp))
            if (!is.na(bl[1,1]) && bl[1,1] > 30 && bl[1,1] < best_break) {
              best_break <- bl[1,1]
            }
          }
          if (best_break <= nchar(fp_chunk)) {
            fp_text <- str_trim(str_sub(fp_chunk, 1, best_break - 1))
          } else {
            fp_text <- str_trim(str_sub(fp_chunk, 1, min(1500L, nchar(fp_chunk))))
          }
          if (nchar(fp_text) >= 20) {
            cat(sprintf("    FRNT Strategy 2: found poem via title word '%s' (%d chars)\n",
                        ftw[1], nchar(fp_text)))
          }
        }
      }
      fp_text_raw <- fp_text  # preserve raw text for FNF dedup
      fp_text <- clean_ocr_body(fp_text)
      if (nchar(fp_text) >= 20) {
        # Use appropriate category prefix (Poem for Poetry, Frnt for Frontispiece)
        cat_prefix <- if (arts$category[i] == "Poetry") "Poem" else "Frnt"
        article_num <- article_num + 1L
        fn <- file.path(out_folder,
          paste0(prefix, "_", sprintf("%02d", article_num), "_", cat_prefix, "_",
                 short_title_filename(arts$title[i]), ".txt"))
        # Add bracket header (same format as regular articles)
        fp_meta_title <- arts$title[i]
        fp_meta_author <- ""
        fp_auth_split <- str_match(arts$title[i],
          "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
        if (!is.na(fp_auth_split[1,1])) {
          fp_meta_title <- str_trim(fp_auth_split[1,2])
          fp_meta_author <- str_trim(fp_auth_split[1,3])
        }
        fp_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                              VOLUME, meta$month, YEAR, arts$page[i],
                              arts$category[i], fp_meta_title, fp_meta_author)
        header_text <- paste0(fp_header, "[", arts$title[i], "]\n\n")
        write_file(paste0(header_text, fp_text), fn)
        # Store RAW text (not cleaned) for FNF dedup — must match fnf_text's raw format
        misc_fp_extracted_text <- c(misc_fp_extracted_text, fp_text_raw)
        cat(sprintf("    Saved [%02d] %s: '%s' (%s chars from misc text)\n",
                    article_num, cat_prefix, str_sub(arts$title[i], 1, 50),
  ...
  if (length(pest_idx) > 0) {
    ...
    if (length(clustered_pest) > 0) {
      ...
      if (nrow(t2_prescan) > 0) {
        ...
        # ---- Poem CAPS-title preference ----
        # find_in_window() often matches the author name (unique words)
        # instead of the ALL-CAPS poem header.  For poems: search backward
        # from the pre-scan position for the ALL-CAPS title.  If found,
        # shift the position to capture the actual poem text.
        for (j in seq_len(nrow(t2_prescan))) {
          ai <- t2_prescan$idx[j]
          if (!isTRUE(arts$category[ai] == "Poetry")) next

          # Strip author from title to get just the poem name
          t_only <- str_replace(arts$title[ai],
            "\\s*,?\\s+[A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){0,3}$", "")
          if (nchar(t_only) < 3) next

          caps_title <- toupper(str_trim(t_only))
          ps_pos <- t2_prescan$position[j]

          # Search backward from pre-scan position (up to 3000 chars)
          caps_lo <- max(ps_pos - 3000L, body_start)
          caps_chunk <- str_sub(body_text, caps_lo, ps_pos - 1L)
          caps_m <- str_locate(caps_chunk, fixed(caps_title))

          if (!is.na(caps_m[1,1])) {
            caps_pos <- caps_lo + caps_m[1,1] - 1L
            caps_pos <- max(
              snap_to_para_start(body_text, caps_pos, body_start),
              body_start)
            if (caps_pos < ps_pos - 100L) {
              cat(sprintf("    PRESCAN CAPS SHIFT: '%s' (pg %d): %d -> %d (back %d)\n",
                          str_sub(arts$title[ai], 1, 40), arts$page[ai],
                          ps_pos, caps_pos, ps_pos - caps_pos))
              t2_prescan$position[j] <- caps_pos
            }
          }
        }
        t2_prescan <- t2_prescan[order(t2_prescan$position), ]
    ...
    # ----------------------------------------------------------
    # T2 pre-scan enrichment: use pre-scanned positions as
    # page-index markers for articles that pre-scan couldn't find.
    # E.g. June: pre-scan found 13/15 clustered articles;
    # the remaining 2 now interpolate from the pre-scan positions
    # instead of the original sparse running-header index.
    # ----------------------------------------------------------
    if (nrow(t2_prescan) >= 2) {
      prescan_enrich <- data.frame(
        page     = arts$page[t2_prescan$idx],
        position = t2_prescan$position,
        stringsAsFactors = FALSE
      )
      combined_pi2 <- rbind(page_index, prescan_enrich)
      combined_pi2 <- combined_pi2[!duplicated(combined_pi2$page), ]
      combined_pi2 <- combined_pi2[order(combined_pi2$position), ]

      non_prescan <- pest_idx[!(pest_idx %in% t2_prescan$idx)]
      non_prescan_pest <- non_prescan[search_method[non_prescan] %in%
                                        c("page_est", "page_est_verse")]

      if (length(non_prescan_pest) > 0 &&
          nrow(combined_pi2) > nrow(page_index)) {
        n_added2 <- nrow(combined_pi2) - nrow(page_index)
        cat(sprintf("  T2 PRESCAN ENRICH: +%d markers (%d -> %d total)\n",
                    n_added2, nrow(page_index), nrow(combined_pi2)))

        for (ri in non_prescan_pest) {
          old_pos <- positions[ri]
          new_est <- page_to_pos(combined_pi2, arts$page[ri],
                                  body_start, body_end)
          if (!is.na(new_est)) {
            new_pos <- max(snap_to_para_start(body_text, new_est,
                                               body_start), body_start)
            if (abs(new_pos - old_pos) > 200) {
              cat(sprintf("    [T2_ENRICH] #%d '%s' (pg %d): %d -> %d (delta %+d)\n",
                          ri, str_sub(arts$title[ri], 1, 40), arts$page[ri],
                          old_pos, new_pos, new_pos - old_pos))
            }
            positions[ri] <- new_pos
        ...
        # Re-sort T2 with updated positions
        pest_sorted <- pest_idx[order(positions[pest_idx])]
        t2_positions <- positions[pest_sorted]
        page_index   <- combined_pi2
      }
    }
  ...
  for (mi in miss_idx) {
    ...
    if (str_detect(misc_text, regex("[\u2014\u2013—-]\\s*[A-Z][a-z]+.*(?:Utah|California|Idaho|England|Canada|Washington|Oregon|Arizona)",
      ...
      # Remove frontispiece poem text from FNF content to avoid duplication.
      # Poems extracted from misc_text (misc_fp_extracted_text) should not
      # also appear in the From Near and Far file.
      if (exists("misc_fp_extracted_text") && length(misc_fp_extracted_text) > 0) {
        for (fp_txt in misc_fp_extracted_text) {
          if (nchar(fp_txt) >= 20 && str_detect(fnf_text, fixed(str_sub(fp_txt, 1, min(60, nchar(fp_txt)))))) {
            fnf_text <- str_trim(str_replace(fnf_text, fixed(fp_txt), ""))
            cat(sprintf("    FNF DEDUP: removed %d chars of frontispiece poem from FNF\n", nchar(fp_txt)))
          }
        }
      }
      ...
      # Update Misc file: remove FNF and frontispiece, keep publishing info + ads
      misc_file <- file.path(out_folder, paste0(prefix, "_00_Misc.txt"))
      ads_collected <- ""
      if (file.exists(misc_file)) {
        old_misc <- read_file(misc_file)
        # Preserve ads in both formats: "=== ADVERTISEMENTS" and "=== OVERLAP TRIM"
        ads_loc <- str_locate(old_misc, fixed("\n\n=== ADVERT"))
        overlap_loc <- str_locate(old_misc, fixed("\n\n=== OVERLAP"))
        earliest_special <- nchar(old_misc) + 1L
        if (!is.na(ads_loc[1,1]) && ads_loc[1,1] < earliest_special) earliest_special <- ads_loc[1,1]
        if (!is.na(overlap_loc[1,1]) && overlap_loc[1,1] < earliest_special) earliest_special <- overlap_loc[1,1]
        if (earliest_special <= nchar(old_misc)) {
          ads_collected <- str_sub(old_misc, earliest_special)
        }
      }
      # Build clean Misc: publishing info + collected ads/overlap
      pub_only <- ""
      if (exists("publishing_text") && nchar(publishing_text) > 10) pub_only <- str_trim(publishing_text)
      new_misc_parts <- character(0)
      if (nchar(pub_only) > 10) new_misc_parts <- c(new_misc_parts, pub_only)
      if (nchar(ads_collected) > 10) new_misc_parts <- c(new_misc_parts, str_trim(ads_collected))
      new_misc <- paste(new_misc_parts, collapse = "\n\n")
      if (nchar(new_misc) > 10) {
        write_file(str_trim(new_misc), misc_file)
        cat(sprintf("    Updated Misc: cleared FNF+cover, kept publishing+ads (%s chars)\n",
                    format(nchar(new_misc), big.mark=",")))
      } else {
        write_file("", misc_file)
        cat("    Cleared Misc (ads will accumulate during extraction)\n")
      }
    }
  }

  # Extract frontispiece from cover/Misc text if not found via misc_frontispiece path
  # The frontispiece poem is typically on the magazine cover (pre-body text).
  # Extract into its own file (don't just count as "found in Misc").
  for (mi in miss_idx) {
    if (arts$category[mi] != "Frontispiece") next
    if (saved_flags[mi]) next  # already extracted
    ...
    # Strategy 1: Find poem via author name near cover/frontispiece markers
    author_last <- ""
    auth_m <- str_match(arts$title[mi], "([A-Z][a-zA-Z]+)\\s*$")
    if (!is.na(auth_m[1,1])) author_last <- str_trim(auth_m[1,1])
      ...
      if (!is.na(marker_loc[1,1])) {
        ...
        if (!is.na(auth_in_post[1,1])) {
          ...
          poem_chunk <- str_sub(misc_text, poem_abs_start,
                                min(nchar(misc_text), poem_abs_start + 3000L))
          end_pats <- c(
            "(?i)Transparency\\s+by", "(?i)Lithographed",
            "(?i)Photograph\\s+by", "(?i)Dick Scopes", "(?i)Mary Scopes",
            "(?i)Relief Society Magazine",
            "(?i)Volume\\s+\\d+\\s+Number",
            "(?i)\\d{4}\\s+Vol\\s+\\d+",
            "(?i)\\bVol\\.?\\s+\\d+,\\s+No",
    ...
    # Strategy 2: Title word search fallback
    if (nchar(fp_text) < 20 && length(fp_words) >= 1) {
      for (w in fp_words) {
        fp_loc <- str_locate(misc_text, regex(paste0("\\b", w, "\\b"), ignore_case=TRUE))
        if (!is.na(fp_loc[1,1])) {
          fp_start <- fp_loc[1,1]
          fp_chunk <- str_sub(misc_text, fp_start, min(nchar(misc_text), fp_start + 2000L))
          break_pats <- c("(?i)Transparency", "(?i)Lithographed",
                          "(?i)Dick Scopes",
                          "(?i)Relief Society Magazine",
                          "(?i)Volume\\s+\\d+\\s+Number",
                          "(?i)\\d{4}\\s+Vol\\s+\\d+",
          ...
          if (nchar(fp_text) >= 20) break
        }
      }
    }

    # Save as separate Frontispiece file
    fp_text <- clean_ocr_body(str_trim(fp_text))
    if (nchar(fp_text) >= 20) {
      article_num <- article_num + 1L
      fn <- file.path(out_folder,
        paste0(prefix, "_", sprintf("%02d", article_num), "_Frnt_",
               short_title_filename(arts$title[mi]), ".txt"))
      fp_meta_title <- arts$title[mi]
      fp_meta_author <- ""
      fp_auth_split <- str_match(arts$title[mi],
        "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
      if (!is.na(fp_auth_split[1,1])) {
        fp_meta_title <- str_trim(fp_auth_split[1,2])
        fp_meta_author <- str_trim(fp_auth_split[1,3])
      ...
      fp_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                            VOLUME, meta$month, YEAR, arts$page[mi],
                            arts$category[mi], fp_meta_title, fp_meta_author)
      header_text <- paste0(fp_header, "[", arts$title[mi], "]\n\n")
      write_file(paste0(header_text, fp_text), fn)
      misc_fp_extracted_text <- c(misc_fp_extracted_text, fp_text)
      cat(sprintf("    Saved [%02d] FRNT: '%s' (%s chars from misc/cover text)\n",
                  article_num, str_sub(arts$title[mi], 1, 50),
                  format(nchar(fp_text), big.mark=",")))
      saved_count <- saved_count + 1L
      saved_flags[mi] <- TRUE
      saved_files[mi] <- fn
    } else {
      # Can't extract poem — content may not be in OCR. Count as found if any word matches.
      found_any <- FALSE
      for (w in fp_words) {
      ...
      if (found_any) {
        saved_count <- saved_count + 1L
        saved_flags[mi] <- TRUE
        cat(sprintf("    FRONTISPIECE '%s' — title words in Misc but poem not extractable\n",
  ...
  for (mi in miss_idx) {
    ...
    if (!is.na(bday_m1[1,1])) {
      ...
      bday_text <- bday_result$clean
      if (nchar(bday_result$ads) > 10L)
        append_ads_to_misc(out_folder, prefix, bday_result$ads, "Birthday Congratulations")
  ...
  for (mi in miss_idx2) {
    ...
    # Try to extract the pre-body poem into its own file (not just count as "found")
    pb_text <- ""

    # Search for title words in misc_text, extract poem section
    for (w in fp_words) {
      wloc <- str_locate(misc_text, regex(paste0("\\b", w, "\\b"), ignore_case=TRUE))
      if (!is.na(wloc[1,1])) {
        pb_start <- wloc[1,1]
        pb_chunk <- str_sub(misc_text, pb_start,
                            min(nchar(misc_text), pb_start + 2000L))
        # End at photo credits, FNF signatures, or other section markers
        break_pats <- c("(?i)Transparency", "(?i)Lithographed",
                        "(?i)Photograph\\s+by", "(?i)Dick Scopes",
                        "\\d{3}\\s+[A-Z]")  # page number + next article start
        best_break <- nchar(pb_chunk) + 1L
        for (bp in break_pats) {
          bl <- str_locate(pb_chunk, regex(bp))
          if (!is.na(bl[1,1]) && bl[1,1] > 30 && bl[1,1] < best_break) {
            best_break <- bl[1,1]
          }
        }
        if (best_break <= nchar(pb_chunk)) {
          pb_text <- str_trim(str_sub(pb_chunk, 1, best_break - 1))
        } else {
          pb_text <- str_trim(str_sub(pb_chunk, 1, min(1500L, nchar(pb_chunk))))
        }
        if (nchar(pb_text) >= 20) break
      }
    }

    pb_text <- clean_ocr_body(str_trim(pb_text))
    if (nchar(pb_text) >= 20) {
      cat_prefix <- if (arts$category[mi] == "Poetry") "Poem" else
                    if (arts$category[mi] == "Frontispiece") "Frnt" else
                    abbrev_category(arts$category[mi])
      article_num <- article_num + 1L
      fn <- file.path(out_folder,
        paste0(prefix, "_", sprintf("%02d", article_num), "_", cat_prefix, "_",
      ...
      pb_meta_title <- title
      pb_meta_author <- ""
      pb_auth_split <- str_match(title,
        "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
      if (!is.na(pb_auth_split[1,1])) {
        pb_meta_title <- str_trim(pb_auth_split[1,2])
        pb_meta_author <- str_trim(pb_auth_split[1,3])
      }
      pb_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                            VOLUME, meta$month, YEAR, pg,
                            arts$category[mi], pb_meta_title, pb_meta_author)
      header_text <- paste0(pb_header, "[", title, "]\n\n")
      write_file(paste0(header_text, pb_text), fn)
      cat(sprintf("    Saved [%02d] PRE-BODY: '%s' (%s chars from misc text)\n",
      ...
      # Can't extract — count as found if any word matches
      found_in_pre <- FALSE
      for (w in fp_words) {
        if (str_detect(misc_text, regex(w, ignore_case=TRUE))) {
      ...
      if (found_in_pre) {
        saved_count <- saved_count + 1L
        saved_flags[mi] <- TRUE
        cat(sprintf("    PRE-BODY '%s' found in Misc text (not extractable as separate file)\n",
                    str_sub(title, 1, 50)))
      }
    }
  }
  ...
  for (mi in miss_idx2) {
    ...
    if (nchar(poem_text) >= 20 && nchar(poem_text) <= 5000) {
      ...
      poem_text_clean <- clean_ocr_body(poem_text)
      # Add metadata header
      p_meta_title <- arts$title[mi]
      p_meta_author <- ""
      p_auth_split <- str_match(arts$title[mi],
        "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
      if (!is.na(p_auth_split[1,1])) {
        p_meta_title <- str_trim(p_auth_split[1,2])
        p_meta_author <- str_trim(p_auth_split[1,3])
      }
      p_meta_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                               VOLUME, meta$month, YEAR, arts$page[mi],
                               arts$category[mi], p_meta_title, p_meta_author)
      write_file(paste0(p_meta_header, poem_text_clean), fn)
      ...
      if (nchar(poem_text) >= 20 && nchar(poem_text) <= 5000) {
        ...
        fn <- file.path(out_folder,
          paste0(prefix, "_", sprintf("%02d", article_num), "_Poem_",
                 short_title_filename(poem_title), ".txt"))
        # Add metadata header
        pr_meta_title <- arts$title[mi]
        pr_meta_author <- ""
        pr_auth_split <- str_match(arts$title[mi],
          "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
        if (!is.na(pr_auth_split[1,1])) {
          pr_meta_title <- str_trim(pr_auth_split[1,2])
          pr_meta_author <- str_trim(pr_auth_split[1,3])
        }
        pr_meta_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                                  VOLUME, meta$month, YEAR, arts$page[mi],
                                  arts$category[mi], pr_meta_title, pr_meta_author)
        write_file(paste0(pr_meta_header, poem_text), fn)
        record_saved(fn, poem_pos, poem_pos + nchar(poem_text) - 1L)
  ...
  if (length(unsaved_idx) > 0) {
    ...
    for (mi in unsaved_idx) {
      ...
      if (nchar(art_text) >= 50) {
        ...
        rc_meta_title <- title
        rc_meta_author <- ""
        rc_auth_split <- str_match(title,
          "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
        if (!is.na(rc_auth_split[1,1])) {
          rc_meta_title <- str_trim(rc_auth_split[1,2])
          rc_meta_author <- str_trim(rc_auth_split[1,3])
        }
        rc_meta_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                                  VOLUME, meta$month, YEAR, arts$page[mi],
                                  cat_type, rc_meta_title, rc_meta_author)
        art_text <- paste0(rc_meta_header, art_text)
        fn <- file.path(out_folder,
          paste0(prefix, "_", sprintf("%02d", article_num), "_",
                 cat_label, "_",
                 short_title_filename(title), ".txt"))
  ...
  if (length(vt_unsaved) > 0) {
    ...
    for (mi in vt_unsaved) {
      ...
      if (nchar(art_text) >= 50) {
        ...
        vt_meta_title <- title
        vt_meta_author <- ""
        vt_auth_split <- str_match(title,
          "^(.{3,})\\s+([A-Z][a-zA-Z.]+(?:\\s+[A-Z][a-zA-Z.]*){1,3})$")
        if (!is.na(vt_auth_split[1,1])) {
          vt_meta_title <- str_trim(vt_auth_split[1,2])
          vt_meta_author <- str_trim(vt_auth_split[1,3])
        }
        vt_meta_header <- sprintf("[Vol%d | %s %d | Page %d | %s | %s | %s]\n",
                                  VOLUME, meta$month, YEAR, arts$page[mi],
                                  arts$category[mi], vt_meta_title, vt_meta_author)
        art_text <- paste0(vt_meta_header, art_text)
        fn <- file.path(out_folder,
          paste0(prefix, "_", sprintf("%02d", article_num), "_",
                 cat_label, "_",
                 short_title_filename(title), ".txt"))
          ...
          if (str_detect(last_char, "[a-zA-Z,;:]")) {
            ...
            if (!is.na(tail_loc[1,1])) {
              ...
              if (!is.na(sent_end[1,1]) && sent_end[1,1] <= 2000) {
                ...
                extension <- clean_vol50_headers(extension)
                extension <- clean_ocr_body(extension)
                extended_text <- paste0(txt, str_trim(extension))
  ...
  if (length(gap_texts) > 0 && length(saved_ranges) > 0) {
    ...
    if (length(saved_ranges) > 0) {
      ...
      for (gi in seq_along(gap_texts)) {
        ...
        # Match to nearest article within 12000 chars (~5 pages).
        # Widened from 5000 to ensure text with clear page numbers stays
        # with the nearest article rather than falling through to Misc.
        matched_file <- NULL
        append_mode <- "after"

        # Section-header redirect: if the gap text begins with a section
        # header like "Lesson Department HOMEMAKING", it belongs to the
        # FOLLOWING article (the lesson), not the preceding one.  This
        # prevents Notes/Activities from re-absorbing Homemaking content
        # that was excluded by the SECTION_BOUNDARY in find_article_end().
        gap_starts_section <- str_detect(str_trim(gt),
          regex("^(?:\\d{2,3}\\s+)?Lesson\\s+Department\\s+(?:HOMEMAKING|SPIRITUAL|VISITING|SOCIAL|CULTURAL)",
                ignore_case = TRUE))
        if (gap_starts_section && !is.na(best_foll) && foll_dist <= 12000L) {
          matched_file <- sr_df$file[best_foll]
          append_mode <- "before"
          cat(sprintf("    GAP SECTION-ROUTE: gap starts with section header → %s (skipping prec)\n",
                      basename(matched_file)))
        } else if (prec_dist <= foll_dist && prec_dist <= 12000L) {
          matched_file <- sr_df$file[best_prec]
          append_mode <- "after"
        } else if (foll_dist < prec_dist && foll_dist <= 12000L) {
          matched_file <- sr_df$file[best_foll]
          append_mode <- "before"
        }

        # Page-marker routing: if the gap text contains page markers
        # (e.g., "237 March 1970"), check if those pages belong to a
        # different, undersized article. If so, redirect the gap to that
        # article instead of the position-nearest one. This fixes cases
        # where OCR text order ≠ physical page order.
        if (!is.null(matched_file)) {
          ...
          pm_matches <- str_extract_all(gt, pm_pat)[[1]]
          if (length(pm_matches) > 0) {
            gap_pages <- as.integer(str_extract(pm_matches, "^\\d{2,3}"))
            gap_pages <- gap_pages[!is.na(gap_pages) & gap_pages > 100]
            if (length(gap_pages) > 0) {
              primary_page <- gap_pages[1]
              # Find which CONTENTS article owns this page
              owner_idx <- NA_integer_
              for (ai in seq_len(nrow(arts))) {
                if (arts$page[ai] <= 0) next
                next_pg <- if (ai < nrow(arts)) arts$page[ai + 1] else arts$page[ai] + 10L
                if (next_pg <= 0) next_pg <- arts$page[ai] + 10L
                if (arts$page[ai] <= primary_page && next_pg > primary_page) {
                  owner_idx <- ai
                  break
                }
              }
              if (!is.na(owner_idx) && saved_flags[owner_idx] &&
                  !is.na(saved_files[owner_idx]) &&
                  file.exists(saved_files[owner_idx]) &&
                  !isTRUE(arts$category[owner_idx] == "Poetry")) {
                owner_file <- saved_files[owner_idx]
                owner_size <- file.info(owner_file)$size
                next_pg <- if (owner_idx < nrow(arts)) arts$page[owner_idx + 1]
                           else arts$page[owner_idx] + 4L
                if (next_pg <= 0) next_pg <- arts$page[owner_idx] + 4L
                expected_pages <- max(1L, next_pg - arts$page[owner_idx])
                expected_size <- expected_pages * 2400L
                if (owner_size < expected_size * 0.4 &&
                    owner_file != matched_file) {
                  cat(sprintf("    GAP PAGE-ROUTE: pg %d -> '%s' (%d chars, expected ~%d) instead of '%s'\n",
                              primary_page, basename(owner_file), owner_size,
                              expected_size, basename(matched_file)))
                  matched_file <- owner_file
                  append_mode <- "after"
                }
              }
            }
        ...
        if (!is.null(matched_file) && file.exists(matched_file)) {
          ...
          # Ad-trim gap text: DISABLED.
          # Both mid-article ad stripping AND trailing ad detection are too
          # aggressive for gap text.  Gap text comes from OCR pages where
          # article content and ads are interleaved on the same page.
          # detect_ad_start finds the first ad-like window and sends
          # everything from that point onward to Misc — but this routinely
          # includes article content that follows or is mixed with ads
          # (e.g., "Jimmy" fiction text in September).  Better to keep
          # some ad text in articles than lose article text to Misc.
          ...
          # All other articles get a generous cap (2x) since gap text from
          # poem-overflow unmarking can be legitimately large continuation text.
          ...
          if (is_poem_file || is_bday_file) {
            ...
            size_cap <- max(existing_len * 2.0, 15000L)
          }
          too_large <- (gap_len > size_cap)

          # Poem/birthday bounce: if gap is too large for a poem neighbor,
          # search for the NEAREST non-poem article in either direction.
          # This handles months with many consecutive poems where both
          # immediate neighbors are poems (e.g., May with 20+ poems).
          if (too_large && (is_poem_file || is_bday_file)) {
            # Search all saved articles for nearest non-poem within distance
            non_poem_candidates <- data.frame(
              idx = integer(0), file = character(0),
              dist = numeric(0), mode = character(0),
              stringsAsFactors = FALSE
            )
            for (sri in seq_len(nrow(sr_df))) {
              sr_bn <- basename(sr_df$file[sri])
              if (str_detect(sr_bn, "_Poem_|_Bday_|_Birthday_")) next
              if (!file.exists(sr_df$file[sri])) next
              # Check distance: gap comes AFTER this article
              if (sr_df$end[sri] < gap_s) {
                d <- gap_s - sr_df$end[sri]
                if (d <= 12000L) {
                  non_poem_candidates <- rbind(non_poem_candidates,
                    data.frame(idx = sri, file = sr_df$file[sri],
                               dist = d, mode = "after", stringsAsFactors = FALSE))
                }
              }
              # Check distance: gap comes BEFORE this article
              if (sr_df$start[sri] > gap_e) {
                d <- sr_df$start[sri] - gap_e
                if (d <= 12000L) {
                  non_poem_candidates <- rbind(non_poem_candidates,
                    data.frame(idx = sri, file = sr_df$file[sri],
                               dist = d, mode = "before", stringsAsFactors = FALSE))
                }
              }
            }
            if (nrow(non_poem_candidates) > 0) {
              # Pick nearest non-poem
              best_np <- non_poem_candidates[which.min(non_poem_candidates$dist), ]
              alt_file <- best_np$file
              alt_mode <- best_np$mode
              alt_existing <- read_file(alt_file)
              alt_cap <- max(nchar(alt_existing) * 2.0, 15000L)
              if (gap_len <= alt_cap) {
                matched_file <- alt_file
                append_mode <- alt_mode
                existing <- alt_existing
                existing_len <- nchar(alt_existing)
                bn <- basename(alt_file)
                is_poem_file <- FALSE
                is_bday_file <- FALSE
                too_large <- FALSE
                cat(sprintf("    GAP POEM-BOUNCE: redirected %s chars from poem to %s (dist %s)\n",
                            format(gap_len, big.mark = ","), basename(alt_file),
                            format(best_np$dist, big.mark = ",")))
              }
            }
          }

          # Gap ad-scoring: DISABLED.
          # The inline ad scorer checks only the first 500 chars of gap text.
          # Gaps that start with subscription info, addresses, or ad-like
          # patterns but contain article content further in get misclassified.
          # Example: March McKay tribute (5K chars) starts with General Board
          # subscription text, scored 11 as "ad", but is article content.
          # Better to keep some ad text in articles than lose article text.
          if (nchar(gt_clean) >= 20L && !too_large) {
            if (append_mode == "after") {
              updated <- paste0(str_trim(existing), "\n\n", gt_clean)
            } else {
              # Prepend gap text AFTER the bracket header so the header
              # stays at the file start.  The gap text is typically the
              # beginning of this article that the title search missed.
              hdr_end <- str_locate(existing, "\\[Vol\\d+[^\\]]*\\]\n")
              if (!is.na(hdr_end[1, 2])) {
                header_part <- str_sub(existing, 1L, hdr_end[1, 2])
                body_part   <- str_trim(str_sub(existing, hdr_end[1, 2] + 1L,
                                                nchar(existing)))
                updated <- paste0(header_part, gt_clean, "\n\n", body_part)
              } else {
                # No bracket header found — plain prepend as fallback
      ...
      if (length(misc_texts) > 0) {
        ...
        for (mti in seq_along(misc_texts)) {
          ...
          for (ti in seq_len(nrow(gap_title_hits))) {
            ...
            # Ad-trim: DISABLED for T3 recovery (same rationale as gap text).
            # Both mid-article and trailing ad detection are too aggressive
            # for recovered text — article content interleaved with ads on
            # OCR pages gets misclassified and sent to Misc.
              ...
              if (gap_title_hits$is_replace[ti]) {
                ...
                if (nchar(t3_text_clean) > old_size) {
                  if (old_size < 500L) {
                    ...
                    # Significant original — append gap content with size cap
                    existing_text <- read_file(old_file)
                    max_append <- as.integer(nchar(existing_text) * 3.0)
                    if (nchar(t3_text_clean) > max_append && max_append > 200L) {
                      cat(sprintf("    T3 SIZE CAP: '%s' — gap %d chars exceeds 3x original %d; capping at %d\n",
                                  str_sub(arts$title[ai], 1, 40), nchar(t3_text_clean),
                                  nchar(existing_text), max_append))
                      overflow_text <- str_sub(t3_text_clean, max_append + 1L, nchar(t3_text_clean))
                      t3_text_clean <- str_sub(t3_text_clean, 1L, max_append)
                      # Send overflow to Misc
                      append_ads_to_misc(out_folder, prefix, overflow_text, "T3 size-cap overflow")
                    }
                    ...
                    cat(sprintf("    T3 APPEND: '%s' (%d + %d = %d chars)\n",
                                str_sub(arts$title[ai], 1, 40), nchar(existing_text),
  ...
  # Ensure Misc file always exists (required: Board, Misc, Contents per month)
  misc_file_final <- file.path(out_folder, paste0(prefix, "_00_Misc.txt"))
  if (!file.exists(misc_file_final)) {
    # Create minimal Misc with publishing boilerplate if available
    if (nchar(misc_text) > 10) {
      write_file(clean_ocr_body(str_trim(misc_text)), misc_file_final)
      cat("  MISC: Created Misc file from pre-body text\n")
    } else {
      write_file(sprintf("[Vol%d | %s %d | Misc]\n", VOLUME, meta$month, YEAR),
                 misc_file_final)
      cat("  MISC: Created placeholder Misc file\n")
    }
  }
  ...
  if (length(saved_ranges_13c) > 0) {
    for (sr_i in seq_along(saved_ranges_13c)) {
      ...
      if (!is.na(cont_ad) && cont_ad > 50L) {
        ...
        cont_text <- str_trim(str_sub(cont_text, 1L, cont_ad - 1L))
        append_ads_to_misc(out_folder, prefix, cont_ad_content,
                           paste0("13c continuation of ", basename(sr$file)))
      }
